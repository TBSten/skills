...
The deterministic parts of the run are shipped as scripts (`bash` / `git` / `curl` / `jq`),
which the agent runs as-is instead of re-deriving the logic each night:

| Script | Purpose |
|--------|---------|
| `scripts/init-run.sh` | Scaffold the run directory, record the start epoch, print `{"date","commit","dir","startedAt"}` |
| `scripts/new-issue.sh <catN> <P0..P3> <slug> [title]` | Atomically allocate the next `<NN>` (parallel-safe, gapless) and generate the issue skeleton |
| `scripts/summary.sh` | Aggregate `issues/*.md` into SUMMARY.md + report format lint as JSON |
| `scripts/check-upstream.sh` | Fetch the cat4 upstream sources and print the version-drift table `[{"tool","latest","project","drift"}]` |

Issue numbering, metadata lines, SUMMARY.md, and the upstream version diff are therefore
script-owned; the AI writes only issue bodies and judges only breaking-change severity.

## Issue format

Each finding is generated by `scripts/new-issue.sh` (never hand-written) at
`.local/tmp/exploratory-nightly-<date>/issues/<NN>-<slug>.md`; the AI fills in the body sections:
...
- `gh` CLI available in the CI environment; `jq` + `curl` for the bundled scripts
- `.local/` in `.gitignore`
