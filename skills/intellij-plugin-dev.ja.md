...
1. **build 基盤を組む** — intellijPlatform (v2) / bundled Kotlin(AA)・Jewel・Compose・Skiko / JBR21 / K2 / since-until を配線し、`compileKotlin`・`buildPlugin`・`runIde` が通るところまで
...
2. **Step 2**: `references/setup/` で build を配線する (basics → preview → snapshot の順)
