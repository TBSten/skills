...
| 戦略 | 説明 | 採用例 |
|------|------|--------|
| **タンデムリリース** | プラグイン = Kotlin バージョン。1 バージョン専用 JAR | kotlinx.serialization, Jetpack Compose |
| **独立リリース** | 1 JAR で複数バージョンをカバー | Metro, Kondition |

### パターン A: Compat Module Layer (metro スタイル)

K2+ のパッチバージョン間の IR API 差異を吸収する。RuntimeにServiceLoader で最適な実装を選択。

```
compiler-plugin/
├── compat/         # Interface + Loader (apiVersion = 対象の最古バージョン)
├── compat-k2000/   # Kotlin 2.0.0–2.0.1x の IR 実装 (minVersion="2.0.0")
├── compat-k2020/   # Kotlin 2.0.20–2.1.x の IR 実装 (minVersion="2.0.20")
└── compat-k23/     # Kotlin 2.2.0+ の IR 実装   (minVersion="2.2.0")
```

**dispatch アルゴリズム**: `minVersion ≤ 現在の Kotlin バージョン` の Factory のうち最大 minVersion を選択。ロード時に `NoClassDefFoundError` が出た Factory は自動スキップ（= そのバージョンで使えない実装が安全に無視される）。

**shadow JAR**: `mergeServiceFiles()` で全 compat の `META-INF/services` を結合し、1 JAR に同梱。

**いつ使うか**: K2 のみ対象で、`IrCall.arguments[param]` → `putValueArgument()` のようなパッチレベルの IR API 差異を吸収したい場合。

**前例**: [ZacSweers/metro](https://github.com/ZacSweers/metro) の `compiler-compat` モジュール

### パターン B: Source Set Separation

K1 (PSI/ComponentRegistrar) と K2 (FIR/CompilerPluginRegistrar) の大きな断絶を吸収する。Gradle がビルド時にソースディレクトリを切り替える。

```
src/main/kotlin/        # 共通 (IR 変換など)
src/v2_0_0/kotlin/      # K2 向け (FIR + CompilerPluginRegistrar, supportsK2=true)
src/pre_2_0_0/kotlin/   # K1 向け (PSI + ComponentRegistrar)
```

- `VersionSpecificAPI` インターフェースを共通側に定義し、各ソースセットで実装
- `KOTLIN_VERSION` 環境変数 or Gradle property で動的に srcDir を切り替え
- K2 向け `CompilerPluginRegistrar` に `supportsK2 = true` が必須

**いつ使うか**: K1/K2 の両方をサポートしたい場合、または FIR と PSI の処理を完全に分離したい場合。

**前例**: [kitakkun/multi-kotlin-support-example](https://github.com/kitakkun/multi-kotlin-support-example), [kitakkun/Kondition](https://github.com/kitakkun/Kondition)

### 組み合わせ

A と B を組み合わせることも可能: K1/K2 の断絶はソースセット分離で、K2 内のパッチ差異はコンパットモジュール層で吸収する。
