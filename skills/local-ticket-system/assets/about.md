...
5. **deferred**: 意図的に後回しにするときは `deferred/` へ移動（再着手の意図あり）
...
5. **deferred**: 着手を先送りにするときは `deferred/` へ移動

chapter は done/closed には移動しない。

## deferred/ のルール

`deferred/` は「今のフェーズでは着手しないが、将来再着手する可能性がある」チケット・チャプターの置き場。`done/` / `closed/` / `archived/` とは異なり、完了を意味しない。
...
`{絵文字}` は内容を表す絵文字を 1 つ選ぶ（例: `🗑️ 不要なコード削除`、`✨ ログイン機能の追加`、`🐛 null pointer in user lookup`）。

## 注意事項

- `.local/` ディレクトリは Git の管理対象外（`.gitignore` に追加）
- チェックリスト末尾の共通項目（不要ファイル削除、検証、セルフレビュー、commit、done 移動）は必ず含める
- チケットタイトル先頭には内容を表す絵文字 prefix を付ける（種別: task / bug / chapter すべて）
