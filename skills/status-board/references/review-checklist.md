...
| レイアウト | body / html の横スクロール、ノード同士の重なり、エピック枠同士の重なり、依存線同士の並走（ほぼ同一経路を長く重なって走り 1 本に見える。単なる交差と端点を共有する合流・分岐は対象外）、block の上向き矢印が間に挟まった別ノードの裏を突き抜けていない、テキストの枠はみ出し、1 ノードの行数過多 |
...
- 線同士がなるべく重ならないレイアウトになっているか（**努力目標**。交差ゼロは目指さない。
  束になって読みづらい箇所があれば overlay の `col` / `anchorY` の調整や edge の削減を 1 回だけ試す。
  それでも残るなら許容してよい）
